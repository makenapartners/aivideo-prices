...
8. Only propose changes for the same kind of thing already being tracked for this provider — matching models, matching operation types (generate vs. edit vs. extend vs. reframe, etc.), matching resolution/tier axes. If the page describes an entirely different pricing dimension that isn't represented anywhere in the current entries (e.g. an HDR tier, a video-editing mode, an extend/reframe feature) and none of the currently-tracked rows are that kind of thing, leave it out — that's a deliberate scope decision made outside this check, not something to propose. Only flag genuinely new resolution tiers or model variants that are clearly the same kind of thing as what's already tracked.

9. If truly nothing changed, return an empty "changes" array. Don't manufacture a change to have something to report.

10. Never propose a change based on anything other than the page text you were given — no outside knowledge, no assumptions about what a provider "probably" charges.
...
async function callClaudeForPriceDiff({ providerName, currentEntries, fetchedPages }) {
  ...
  try {
    ...
    // Include stop_reason and usage in the error -- an empty or
    // truncated response is usually explained by one of these (e.g.
    // stop_reason "max_tokens" means the response got cut off before
    // finishing, which points at a fix -- raising max_tokens -- rather
    // than a one-off API hiccup).
    throw new Error(
      `Could not parse Claude's response as JSON: ${err.message}. ` +
        `stop_reason: ${data.stop_reason}, output_tokens: ${data.usage?.output_tokens}. ` +
        `Raw: ${cleaned.slice(0, 500) || "(empty)"}`
    );
  }
